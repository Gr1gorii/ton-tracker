...
v0.23.2 adds a capture-bound local transaction BOC ledger. Each read reparses
...
v0.23.4 adds an account perspective over verified internal-message native TON
value while keeping every counterparty and movement explicitly observational.
v0.23.5 binds those observations to one canonical network-scoped Toncoin asset
identity with fixed nanoton base-unit semantics.
v0.23.6 groups observed endpoints by canonical network/account identity while
...
v0.23.9 consumes that unchanged merge through
`ton_native_activity_dedup_v1`. The first deterministic occurrence of an
activity identity becomes canonical; all suppressed source coordinates remain
...
| Native TON flow observations | Account-relative classification of verified internal-message value | Incoming/outgoing/self nanotons and observed header counterparty only; not an authoritative transfer ledger or PnL source |
...
- No semantic stitching across unrelated activities. Native activity merge and
  content-addressed deduplication are explicit operations and never inferred
  from interval adjacency alone.
