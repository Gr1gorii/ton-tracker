# TON Wallet Intelligence Dashboard - v0.11.6 LIVE GUARDS Promotion Checklist

Operational checklist for promoting the wallet activity live-provider guards
milestone branch.

## Promotion Gates

- Product label shows `v0.11.6 LIVE GUARDS` in the dashboard header and release
  readiness card.
...
- Guarded live TonAPI wallet activity fetches and persists only account jetton
  balance snapshots.
- Transfers, transactions, swaps, native TON balance, PnL, clustering, and
  ownership proof remain unavailable and visible.
...
- Provider config tests pass from `backend/`:

```bash
.venv/bin/python -m pytest tests/test_providers_config.py -q
```
...
- Browser QA passes on desktop and mobile:
  ...
  - stale current product labels such as `v0.11.5 SCAFFOLDS`,
    `v0.11.4 ADAPTERS`, `v0.11.3 INGEST UI`, `v0.11.2 MOCK INGEST`,
    `v0.11.1 SCHEMA`, `v0.11.0 PLAN`, `v0.10.7`, or `v0.2.1` do not appear as
...
- Guarded live mode may return `data_mode=real` and `source_status=live`, but
  it must only persist jetton balance snapshot rows.
- Non-jetton requested surfaces must remain in `unavailable_surfaces`.
...
```bash
git checkout main
git merge --no-ff v0.11.6-wallet-ingestion-live-provider-guards -m "Merge v0.11.6 wallet ingestion live provider guards"
git tag v0.11.6
...
If the next track begins guarded TonAPI balance coverage expansion:
