...
WALLET_CASE_SYNC_MANIFEST_REVISION = "20260827_0027"
WALLET_CASE_STREAM_CHECKPOINT_REVISION = "20260828_0028"
CURRENT_REVISION = WALLET_CASE_STREAM_CHECKPOINT_REVISION
...
WALLET_CASE_SYNC_MANIFESTS_TABLE = "wallet_case_sync_manifests"
WALLET_CASE_STREAM_CHECKPOINTS_TABLE = "wallet_case_stream_checkpoints"
...
def _create_wallet_case_0015_without_indexes(connection) -> None:
    """Create the frozen 0015 case table while current models target 0025."""
    ...
    table = Table(
        WALLET_CASES_TABLE,
        metadata,
        *migration._case_columns(),
        *migration._constraints(migration._CASE_CHECKS),
    )
    connection.execute(CreateTable(table))
...
def test_fresh_sqlite_reaches_head_with_full_schema_parity(tmp_path):
    ...
    assert report.applied_revisions == (
        ...
        WALLET_CASE_SYNC_MANIFEST_REVISION,
        WALLET_CASE_STREAM_CHECKPOINT_REVISION,
    )
...
def test_wallet_cases_migration_repairs_exact_empty_partial_sqlite_ddl(tmp_path):
    ...
    _upgrade_to_revision(engine, OWNERSHIP_NETWORK_SCOPE_REVISION)
    with engine.begin() as connection:
        _create_wallet_case_0015_without_indexes(connection)
...
def test_wallet_cases_migration_rejects_pre_revision_case_rows(tmp_path):
    ...
    with engine.begin() as connection:
        _create_wallet_case_0015_without_indexes(connection)
        connection.exec_driver_sql(
            "INSERT INTO wallet_cases "
            "(public_id, owner_scope_id, network, data_environment, "
            "canonical_wallet_key, canonical_identity_version, display_address, "
...
def test_wallet_case_metadata_migration_versions_existing_rows(tmp_path):
    engine = _engine(tmp_path / "wallet-case-metadata.db")
    _upgrade_to_revision(engine, WALLET_CASE_LIFECYCLE_REVISION)
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "INSERT INTO wallet_cases "
...
def test_wallet_case_metadata_migration_resumes_exact_added_column(tmp_path):
    engine = _engine(tmp_path / "wallet-case-metadata-resume.db")
    _upgrade_to_revision(engine, WALLET_CASE_LIFECYCLE_REVISION)
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "ALTER TABLE wallet_cases ADD COLUMN metadata_version "
...
@pytest.mark.parametrize(
    ("column_ddl", "error"),
    [
        ("INTEGER DEFAULT '1'", "differs from revision 0025"),
        ("TEXT DEFAULT '1' NOT NULL", "differs from revision 0025"),
    ],
)
def test_wallet_case_metadata_migration_rejects_wrong_column(
    tmp_path,
    column_ddl,
    error,
):
    engine = _engine(tmp_path / f"wallet-case-metadata-wrong-{column_ddl[:4]}.db")
    _upgrade_to_revision(engine, WALLET_CASE_LIFECYCLE_REVISION)
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "ALTER TABLE wallet_cases ADD COLUMN metadata_version " + column_ddl
        )
    ...
    with engine.connect() as connection:
        assert connection.exec_driver_sql(
            "SELECT version_num FROM alembic_version"
        ).scalar_one() == WALLET_CASE_LIFECYCLE_REVISION
    engine.dispose()


def test_wallet_case_metadata_migration_rejects_invalid_existing_version(tmp_path):
    engine = _engine(tmp_path / "wallet-case-metadata-invalid-row.db")
    _upgrade_to_revision(engine, WALLET_CASE_LIFECYCLE_REVISION)
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "ALTER TABLE wallet_cases ADD COLUMN metadata_version "
...
def test_wallet_case_metadata_migration_is_forward_only(tmp_path):
    engine = _engine(tmp_path / "wallet-case-metadata-forward-only.db")
...
def test_wallet_case_sync_manifest_migration_creates_exact_schema(tmp_path):
    engine = _engine(tmp_path / "wallet-case-sync-manifest.db")
    _upgrade_to_revision(engine, WALLET_CASE_CATALOG_REVISION)

    report = run_database_migrations(engine)

    assert report.revision_before == WALLET_CASE_CATALOG_REVISION
    assert report.revision_after == CURRENT_REVISION
    assert report.applied_revisions == (
        WALLET_CASE_SYNC_MANIFEST_REVISION,
        WALLET_CASE_STREAM_CHECKPOINT_REVISION,
    ...
    assert [
        column["name"]
        for column in inspector.get_columns(WALLET_CASE_SYNC_MANIFESTS_TABLE)
    ] == [
        "id",
        "public_id",
        "case_sync_id",
    ...
    assert {
        item["name"]: (tuple(item["column_names"]), bool(item["unique"]))
        for item in inspector.get_indexes(WALLET_CASE_SYNC_MANIFESTS_TABLE)
    } == {
    ...
    foreign_key = inspector.get_foreign_keys(
        WALLET_CASE_SYNC_MANIFESTS_TABLE
    )[0]
    assert tuple(foreign_key["constrained_columns"]) == ("case_sync_id",)
    assert foreign_key["referred_table"] == WALLET_CASE_SYNCS_TABLE
    assert tuple(foreign_key["referred_columns"]) == ("id",)
    assert foreign_key["options"]["ondelete"] == "CASCADE"
    _assert_schema_matches_models(engine)
    engine.dispose()
...
def test_wallet_case_sync_manifest_migration_resumes_exact_empty_table(tmp_path):
    engine = _engine(tmp_path / "wallet-case-sync-manifest-resume.db")
    _upgrade_to_revision(engine, WALLET_CASE_CATALOG_REVISION)
    models.WalletCaseSyncManifest.__table__.create(bind=engine)
...
def test_wallet_case_sync_manifest_migration_never_adopts_existing_rows(
    tmp_path,
):
    engine = _engine(tmp_path / "wallet-case-sync-manifest-row-adoption.db")
    _upgrade_to_revision(engine, WALLET_CASE_CATALOG_REVISION)
    wallet_cases = database.Base.metadata.tables[WALLET_CASES_TABLE]
    case_syncs = database.Base.metadata.tables[WALLET_CASE_SYNCS_TABLE]
    manifests = database.Base.metadata.tables[WALLET_CASE_SYNC_MANIFESTS_TABLE]
    models.WalletCaseSyncManifest.__table__.create(bind=engine)
    digest = "a" * 64
    with engine.begin() as connection:
        case_id = connection.execute(
...
def test_wallet_case_sync_manifest_migration_rejects_wrong_index(tmp_path):
    engine = _engine(tmp_path / "wallet-case-sync-manifest-wrong-index.db")
    _upgrade_to_revision(engine, WALLET_CASE_CATALOG_REVISION)
    models.WalletCaseSyncManifest.__table__.create(bind=engine)
    with engine.begin() as connection:
        connection.exec_driver_sql(
            "DROP INDEX uq_wallet_case_sync_manifests_public_id"
        )
        connection.exec_driver_sql(
            "CREATE INDEX uq_wallet_case_sync_manifests_public_id "
            "ON wallet_case_sync_manifests(public_id)"
        )

    with pytest.raises(RuntimeError, match="differs from revision 0027"):
        run_database_migrations(engine)
    engine.dispose()


def test_wallet_case_sync_manifest_cascades_with_sync(tmp_path):
    engine = _engine(tmp_path / "wallet-case-sync-manifest-cascade.db")
    run_database_migrations(engine)
    wallet_cases = database.Base.metadata.tables[WALLET_CASES_TABLE]
    case_syncs = database.Base.metadata.tables[WALLET_CASE_SYNCS_TABLE]
    manifests = database.Base.metadata.tables[WALLET_CASE_SYNC_MANIFESTS_TABLE]
    ...
    with engine.begin() as connection:
        ...
        remaining = connection.execute(
            manifests.select().where(manifests.c.case_sync_id == sync_id)
        ).all()
...
def test_wallet_case_sync_manifest_migration_is_forward_only(tmp_path):
    engine = _engine(tmp_path / "wallet-case-sync-manifest-forward-only.db")
...
def test_wallet_case_stream_checkpoint_migration_creates_exact_schema(tmp_path):
    engine = _engine(tmp_path / "wallet-case-stream-checkpoint.db")
    _upgrade_to_revision(engine, WALLET_CASE_SYNC_MANIFEST_REVISION)

    report = run_database_migrations(engine)

    assert report.revision_before == WALLET_CASE_SYNC_MANIFEST_REVISION
    assert report.revision_after == CURRENT_REVISION
    assert report.applied_revisions == (WALLET_CASE_STREAM_CHECKPOINT_REVISION,)
    inspector = inspect(engine)
    assert [
        column["name"]
        for column in inspector.get_columns(WALLET_CASE_STREAM_CHECKPOINTS_TABLE)
    ] == [
        "id",
        "public_id",
        "case_id",
    ...
    assert {
        item["name"]: (tuple(item["column_names"]), bool(item["unique"]))
        for item in inspector.get_indexes(WALLET_CASE_STREAM_CHECKPOINTS_TABLE)
    } == {
    ...
    assert {
        ...
        (("case_id",), WALLET_CASES_TABLE, ("id",), "CASCADE"),
        (("source_sync_id",), WALLET_CASE_SYNCS_TABLE, ("id",), "CASCADE"),
    }
    _assert_schema_matches_models(engine)
    engine.dispose()


def test_wallet_case_stream_checkpoint_migration_resumes_exact_empty_table(
    tmp_path,
):
    engine = _engine(tmp_path / "wallet-case-stream-checkpoint-resume.db")
    _upgrade_to_revision(engine, WALLET_CASE_SYNC_MANIFEST_REVISION)
...
def test_wallet_case_stream_checkpoint_migration_is_forward_only(tmp_path):
    engine = _engine(tmp_path / "wallet-case-stream-checkpoint-forward-only.db")
    _upgrade_to_revision(engine, WALLET_CASE_STREAM_CHECKPOINT_REVISION)
    with engine.begin() as connection:
        with pytest.raises(RuntimeError, match="intentionally unsupported"):
            command.downgrade(
                migration_config(connection),
